...
PINCODE = "0"
while len(PINCODE) != 6:
    PINCODE = input("Enter the pincode which you want to status => ")
    if len(PINCODE) < 6:
...
# REQ_DATE = "31-03-2022"
REQ_DATE = input("Enter the Date to get status (Date format: DD-MM-YY => ")
...
if Total_centers != 0:
    ...
    print(f"Unfortunately !! Seems like no center in this area / kindly re-cheak the Pincode")
...
for cent in range(Total_centers):
    ...
    for _sess in range(len(this_session)):
        print("{0:^12} {1:^12} {2:^13} {3:^14} {4:^16} ".format(
            this_session[_sess]['date'], this_session[_sess]['vaccine'], fee_val, this_session[_sess]['min_age_limit'], this_session[_sess]['available_capacity']))
